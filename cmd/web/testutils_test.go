...
import (
	"github.com/TeslaMode1X/snippetbox/pkg/models/mock"
...
// The final parameter to this method is an url.Values object which can contain
// any data that you want to send in the request body.
func (ts *testServer) postForm(t *testing.T, urlPath string, form url.Values) (int, http.Header, []byte) {
	// Send a POST request to the test server with the provided URL path and form data.
	rs, err := ts.Client().PostForm(ts.URL+urlPath, form)
	if err != nil {
		t.Fatal(err) // Log the error and stop the test if the request fails.
	}

	// Read the response body.
	defer rs.Body.Close()                // Ensure the response body is closed after reading.
	body, err := ioutil.ReadAll(rs.Body) // Read the entire response body.
	if err != nil {
		t.Fatal(err) // Log the error and stop the test if reading the body fails.
	}

	// Return the response status code, headers, and body.
	return rs.StatusCode, rs.Header, body
}
